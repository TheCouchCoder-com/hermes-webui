...
// Fork helper: format a per-target "update check unavailable" status line for
// the Settings → System update display (panels.js). Independent of the banner
// design — kept after dropping the fork's split banner in favor of upstream's.
...
// app titlebar (#profileChipTitlebar) render the active profile and drive the
// same dropdown. This helper keeps both labels (+ titlebar a11y) in sync and is
// used by boot.js/panels.js. syncTopbar() additionally sets the composer label
// inline so the #3635 source-of-truth gate can verify it reads S.activeProfile.
...
function syncTopbar(){
  if(!S.session){
    ...
    // Update profile chip even when no session is active (e.g. right after profile
    // switch). The chip is the profile-SWITCHER trigger and must reflect the
    // client active profile, not a loaded session's profile (#3635). The composer
    // label is set inline (so the #3635 source-of-truth gate can verify it reads
    // S.activeProfile); _setProfileChipLabel then keeps the fork's titlebar chip
    // (+ a11y) in sync. Guarded so the test harness in
    // test_issue1771_session_model_switch_sync.py (which evals syncTopbar alone)
    // doesn't ReferenceError when the helper isn't pulled in.
    const profileLabel=$('profileChipLabel');
    if(profileLabel) profileLabel.textContent=S.activeProfile||'default';
    if(typeof _setProfileChipLabel==='function') _setProfileChipLabel(S.activeProfile||'default');
    // Also keep upstream's own titlebar profile display (#titlebarProfileLabel,
    // gated by _showTitlebarProfile) in sync -- a distinct element from the
    // fork's titlebar chip above, see _applyTitlebarProfileVisibility in boot.js.
  ...
  // Update profile chip labels (composer + titlebar). Per #3635 the chip is the
  // profile-SWITCHER trigger and governs where the next message / new chat
  // routes, so it must reflect S.activeProfile, NOT the loaded session's
  // profile (opening a cross-profile session must not make the chip disagree
  // with the dropdown checkmark). Set the composer label inline and keep the
  // fork's titlebar chip (+ a11y label) in sync alongside it.
  const profileLabel=$('profileChipLabel');
  if(profileLabel) profileLabel.textContent=S.activeProfile||'default';
  if(typeof _setProfileChipLabel==='function') _setProfileChipLabel(S.activeProfile||'default');
