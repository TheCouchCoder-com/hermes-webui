...
function syncTopbar(){
  ...
  // syncTopbar in isolation). Use upstream's value precedence
  // (session profile > active profile) but route it through the fork helper
  // so both the composer chip and the titlebar chip (+ a11y label) stay in sync.
  if(typeof _setProfileChipLabel==='function') _setProfileChipLabel((S.session&&S.session.profile)||S.activeProfile||'default');
}
