...
def _add_local_origin(repo):
    """Point the repo's ``origin`` at a LOCAL bare repo (not a network URL).

    The fork restricts the release channel to origin-published tags
    (``api/updates._origin_release_tags`` runs ``git ls-remote origin``). A real
    github URL makes these tests depend on which tags upstream happens to publish
    (and on network reachability), which is exactly the flakiness the filter is
    meant to avoid. A local bare origin lets us publish precisely the repo's own
    tags, so the real ls-remote filter path is exercised hermetically.
    """
...
def _publish_tags(repo):
    """Push the repo's tags to its local origin so the origin-tag filter sees them."""
    _git(repo, 'push', '-q', '--force', 'origin', '--tags')
...
def channel_repo(tmp_path):
    ...
    # Publish every tag the fixture created so the fork's origin-published-tag
    # filter treats them as available (mirrors a real fork whose origin serves
...
def test_experimental_user_sees_unpromoted_exp_commit(channel_repo):
    ...
    _publish_tags(channel_repo)  # origin must serve the new exp tag for the filter
    _git(channel_repo, 'checkout', '-q', 'v0.52.5')
...
def test_agent_resolution_identical_under_both_webui_channels(tmp_path, monkeypatch):
    ...
    _publish_tags(agent)                     # origin serves the agent's own release tags
