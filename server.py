...
from api.startup import auto_install_agent_deps, fix_credential_permissions, log_startup_migration
...
def main() -> None:
    ...
    log_startup_migration()  # issue #2: RBAC first-boot migration + legacy-session cleanup

    # #1558 startup self-heal: restore any session JSON truncated below its .bak
    # (data-loss shape #1558). Safe to run unconditionally — clean install is a no-op.
    try:
