...
def _webui_version_now() -> str:
    """Resolve WEBUI_VERSION for code running inside this module.

    A bare `WEBUI_VERSION` reference would be a NameError here: PEP 562
    module __getattr__ (above) only intercepts external `module.WEBUI_VERSION`
    attribute access, not a bare-name LOAD_GLOBAL from code running inside
    this module. `globals()` is this module's own __dict__, so it also picks
    up a test's `monkeypatch.setattr(updates, 'WEBUI_VERSION', ...)`.
    """
    g = globals()
    if 'WEBUI_VERSION' in g:
        return g['WEBUI_VERSION']
    return _cached_version('webui', _detect_webui_version)
...
def channel_version_badge(channel=None) -> str:
    ...
    return _webui_version_now()


def _origin_release_tags(path, channel=DEFAULT_UPDATE_CHANNEL):
    """Return the set of channel tags published on origin, or None if origin cannot be queried.

    Differentiates "origin has no matching tags" (empty set) from "we couldn't ask
    origin" (None). _release_tags uses the former to fall through to the branch check
    and the latter to preserve the legacy local-tag view. Channel-aware so the
    experimental channel filters against ``exp-v*`` tags rather than ``v*``.
    """
    glob = _channel_tag_glob(channel)
    out, ok = _run_git(['ls-remote', '--tags', '--refs', 'origin', glob], path, timeout=10)
    if not ok:
        return None
    tags = set()
...
def _release_tags(path, channel=DEFAULT_UPDATE_CHANNEL):
    """Return the channel's release tags published on origin, newest-first via version-sort.
    ...
    origin_tags = _origin_release_tags(path, channel)
...
def _check_webui_published_release_update():
    ...
    current_version = str(_webui_version_now() or '').strip()
...
def _select_apply_compare_ref(path, channel=DEFAULT_UPDATE_CHANNEL, target=None):
    ...
    if tags:
        ...
        # Fork guard — mirror the check-side fall-through for sync-merged
        # upstream tags: if HEAD's nearest tag isn't origin-published,
        # "advance to the fork's latest tag" would be a downgrade. Drop to the
        # branch ref so apply lands at the same place the check pointed at.
        if not (current_tag and current_tag not in tags):
            behind = _release_gap(tags, current_tag, latest_tag)
            # Mirror the check side exactly: fall through to the branch
            # comparison whenever the checkout has already moved past the
            # release tag the banner would otherwise advertise.
            #
            # Case A — behind == 0 AND HEAD has moved past its nearest tag
            # (the agent repo's day-to-day state between tagged releases):
            # applying the tag would no-op, restart, and re-show the banner.
            #
            # Case (#3140) — behind > 0 but HEAD already CONTAINS the newer
            # latest tag (main-tracking checkouts that fetched a newer tag
...
def _check_repo_release(path, name, channel=DEFAULT_UPDATE_CHANNEL):
    ...
    if current_tag is None:
        ...
        if name == "webui":
            current_version_display = _webui_version_now()
