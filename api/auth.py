...
def _session_expiry(record) -> float | None:
    ...
    Record shapes accepted (issue #2 schema bump + trusted-header SSO):
      - float / int              — legacy single-password session
      - dict {expiry, user_id?, auth_type?, username?, bound_profile?}

    Returns None for anything unparsable so callers can distinguish "no
    valid expiry" from a genuinely-expired timestamp.
    """
...
def create_session(
    ...
    *,
    ...
    `user_id` binds the session to a multi-user record (issue #2).
    `auth_type`/`username`/`bound_profile` bind a trusted-header SSO session
    (see `ensure_trusted_auth_session`). None of the above is accepted for
    backwards compatibility with legacy single-password code paths and
    tests, and stores the session as a bare expiry float."""
    token = secrets.token_hex(32)
    expiry = time.time() + _resolve_session_ttl()
    record: float | dict
    if any(value is not None for value in (user_id, auth_type, username, bound_profile)):
        ...
        if bound_profile is not None:
            record['bound_profile'] = bound_profile
    else:
        record = expiry
    with _SESSIONS_LOCK:
