...
- **PR #TBD** — Account usage/quota support for `opencode-go` provider. Adds `opencode-go` to `_ACCOUNT_USAGE_PROVIDERS` and implements a subprocess-level fallback in `_ACCOUNT_USAGE_SUBPROCESS_CODE` that checks for `OPENCODE_GO_API_KEY` and returns an unavailable snapshot (OpenCode Go does not currently expose a usage API). Includes three new regression tests in `tests/test_provider_quota_status.py` covering missing key, present key, and `get_provider_quota()` integration paths.
